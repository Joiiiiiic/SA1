...
ilts = zeros(1,length(alpha));
iuts = zeros(1,length(alpha));
...
%    boundary layer solver
  [iunt, iuls, iutr, iuts(nalpha), delstaru, thetau] = bl_solv ( su, cpu );
...
%    boundary layer solver
  [ilnt, ills, iltr, ilts(nalpha), delstarl, thetal] = bl_solv ( sl, cpl );
